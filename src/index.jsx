        ...
        import {
            ...
            Briefcase, Heart, Users, FileBadge, Coins, Activity, Bug, ServerOff, Fingerprint, FolderLock, Search
        } from 'lucide-react';
        ...
        const TRANSLATIONS = {
            bg: {
                ...
                live: "НА ЖИВО",
                threatFeedTitle: "БЮЛЕТИН: ЗЛОВРЕДНИ РЕСУРСИ",
                ...
                model: "МОДЕЛ",
                searchPlaceholder: "Търсене на домейн..."
            },
            en: {
                home: "Home",
                tips: "Tips",
                ...
                headerSubtitle: "General Directorate Combating Organised Crime, Bulgarian Ministry of the Interior",
                videoTitle: "Video Channel",
                tipsTitle: "CYBERSECURITY TIPS",
                crimeTypesTitle: "THREATS",
                ...
                ctaMvr: "at GDCOC-MoI",
                footerTitle: "Cybercrime Directorate, General Directorate Combating Organised Crime, Bulgarian Ministry of the Interior",
                address: 'Sofia, 1 Alexander Malinov Blvd, 7th police station, National police HQ and American college main barrier',
                openMap: "Open address in navigation",
                rights: "All rights reserved.",
                latestFrom: "LATEST FROM",
                live: "LIVE",
                threatFeedTitle: "THREAT FEED: MALICIOUS LINKS",
            ...
            return (
                <nav className={`fixed top-0 left-0 w-full z-40 transition-all duration-300 ${isScrolled ? 'bg-slate-900/90 backdrop-blur-md shadow-lg border-b border-gdbop-blue/20' : 'bg-transparent'}`}>
                    <div className="container mx-auto px-4 py-4 flex items-center justify-between">
                        {/* 1. Logo Section (Left) */}
                        {/* 1. Logo Section (Left) */}
                        <div className="flex-shrink-0 flex items-center gap-2 cursor-pointer w-[220px]" onClick={() => scrollToSection('home')}>
                            <ShieldCheck className="text-gdbop-blue" size={28} />
                            <span className="font-bold text-white tracking-wider hidden xl:block">CYBERCRIME.BG</span>
                        </div>

                        {/* 2. Navigation Items (Centered) - Desktop Only */}
                        <div className="hidden lg:flex flex-1 justify-center items-center gap-0.5 xl:gap-1 px-1">
                            {navItems.map((item) => (
                                item.isLink ? (
                                    <a
                                        key={item.id}
                                        href={item.id}
                                        className="px-1.5 py-2 text-[10px] xl:text-[11px] 2xl:text-xs font-bold text-slate-300 hover:text-white hover:bg-white/10 rounded-full transition-all flex items-center gap-1 group whitespace-nowrap"
                                    >
                                        <item.icon size={13} className="text-police-red group-hover:scale-110 transition-transform" />
                                        {item.label}
                                    </a>
                                ) : (
                                    <button
                                        ...
                                        className="px-1.5 py-2 text-[10px] xl:text-[11px] 2xl:text-xs font-bold text-slate-300 hover:text-white hover:bg-white/10 rounded-full transition-all flex items-center gap-1 group whitespace-nowrap"
                                    >
                                        <item.icon size={13} className="text-gdbop-blue group-hover:scale-110 transition-transform" />
                                        {item.label}
                                    </button>
                                )
                            ))}
                        ...
                        <div className="hidden lg:flex flex-shrink-0 items-center justify-end gap-3 px-3 py-1.5 bg-slate-900/80 backdrop-blur-md border border-slate-700/50 rounded-full">
                            ...
                            <a
                                ...
                                className="flex items-center gap-2 border-r border-slate-700 pr-4 opacity-70 hover:opacity-100 transition-opacity group/gdcoc"
                            >
                                <img
                                ...
                                />
                                <span className="text-[10px] xl:text-xs text-slate-400 group-hover/gdcoc:text-slate-200 transition-colors uppercase tracking-wide whitespace-nowrap">
                                    {t.partOf}
                                </span>
                            </a>
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 border border-slate-700/50 rounded-full text-gdbop-blue hover:bg-gdbop-blue hover:text-white hover:border-white transition-all duration-300 group"
                            >
                    ...
                    {isMenuOpen && (
                        <div className="md:hidden absolute top-full left-0 w-full bg-slate-900/95 backdrop-blur-xl border-t border-slate-800 p-4 flex flex-col gap-2 shadow-2xl max-h-[85vh] overflow-y-auto">
                            {navItems.map((item) => (
                                item.isLink ? (
                                    <a
                                        key={item.id}
            ...
            return (
                <section id="phishing-feed" className="container mx-auto px-4 py-16 z-10 relative">
                    ...
                    <div className="glass-panel bg-slate-900/80 backdrop-blur-md rounded-xl border border-gdbop-blue/20 overflow-hidden shadow-[0_0_30px_rgba(71,168,246,0.1)]">
                        <div className="p-4 bg-gdbop-blue/10 border-b border-gdbop-blue/20 flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="flex items-center gap-2 text-gdbop-blue w-full sm:w-auto">
                                <span className="relative flex h-3 w-3">
                            ...
                            <div className="relative w-full sm:max-w-xs md:max-w-sm lg:max-w-md">
                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <Search size={14} className="text-gdbop-blue/50" />
                                </div>
                                <input
                                    type="text"
                                    ...
                                    className="w-full bg-slate-900/50 border border-gdbop-blue/30 text-slate-300 text-xs font-mono rounded-full pl-9 pr-4 py-2 focus:outline-none focus:border-gdbop-blue/70 focus:bg-slate-900/80 transition-all placeholder:text-slate-500"
                                />
                            </div>

                            <div className="text-xs text-slate-400 font-mono w-full sm:w-auto text-right hidden md:block">
                            ...
                            <table className="w-full text-left text-sm text-slate-300">
                                <thead className="bg-slate-950/50 text-slate-400 font-mono uppercase text-xs">
                                    <tr>
                                        ...
                                        <th className="p-4 hidden md:table-cell">{t.model}</th>
                                        <th className="p-4 hidden md:table-cell">{t.source}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-800 font-mono">
                                    {loading ? (
                                        <tr>
                                            <td colSpan="5" className="p-8 text-center text-gdbop-blue animate-pulse">
                                                {t.loading}
                                            </td>
                                        </tr>
                                    ) : error ? (
                                        ...
                                        currentItems.map((item, idx) => (
                                            <tr key={idx} className="hover:bg-gdbop-blue/5 transition-colors">
                                                ...
                                                <td className="p-4 hidden md:table-cell text-xs font-mono text-slate-400">
                                                    {item.llm_analysis?.model || '-'}
                                                </td>
                                                <td className="p-4 hidden md:table-cell text-slate-500">
                                                    {item.source}
                                                </td>
